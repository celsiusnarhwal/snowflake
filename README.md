...
| `SNOWFLAKE_REDIS_URL`            | String   | A [Redis](https://redis.io/) connection string. Snowflake uses Redis to store important authorization data; if this variable is provided, Snowflake will use the Redis database it points to rather than its own internal one, allowing that data to persist across container restarts. Must begin with `redis://` or `rediss://`.                                                               | Yes if using a `-noredis` image; no otherwise | N/A         |
