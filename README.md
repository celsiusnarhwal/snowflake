...
| `SNOWFLAKE_RETURN_TO_REFERRER`      | Boolean  | If this is `true` and the user denies an authorization request, Snowflake will attempt to redirect to the URL from which the authorization request was initiated.[^3]. Snowflake's default behavior in this situation is dictated by [OpenID Connect Core 1.0 § 3.1.2.6](https://openid.net/specs/openid-connect-core-1_0.html#AuthError).                                                            |                           |
